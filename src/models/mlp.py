...
def evaluate_and_visualize_mlp(
    ...
    for i, target in enumerate(y_features):
        ...
        results.append({
            ...
            "roc_auc": auc,
            "pr_auc": ap,
            "precision": precision_score(y_test[target], pred_matrix[:, i], zero_division=0),
            "recall": recall_score(y_test[target], pred_matrix[:, i], zero_division=0),
            "f1": f1_score(y_test[target], pred_matrix[:, i], zero_division=0),
            "accuracy": accuracy_score(y_test[target], pred_matrix[:, i]),
        })
